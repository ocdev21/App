...
**AWS Bedrock Migration (November 2025)**: The AWS EKS deployment now uses Amazon Bedrock Nova Pro instead of local GGUF models. This change:
- Eliminates 10GB model storage requirements (saves ~$3/month EFS costs)
- Reduces container memory footprint from 4Gi to 2Gi
- Enables faster cold starts (no 30-60 second model loading)
- Provides managed AI scaling and reliability via AWS
- Uses IAM Roles for Service Accounts (IRSA) for secure API access
- Maintains API compatibility - no frontend changes required
- OpenShift deployment continues using local TSLAM-4B model

**AWS Cost Control (November 2025)**: For users running 4-hour sessions, automated start/stop scripts have been created:
- `start-all-services.sh` - Quick (5 min) or full (25 min) startup
- `stop-all-services.sh` - Quick stop (~$72/month idle) or full stop (~$18/month base)
- `check-costs.sh` - Real-time cost monitoring
- Per 4-hour session cost: ~$2-3
- Monthly cost for weekly usage: ~$18 base (with proper shutdown)
- ClickHouse uses EBS gp3 volumes (~$5/month persistent storage)
...
- **Databases**:
    ...
    - ClickHouse
- **AI Models**:
    - **OpenShift Deployment**: TSLAM-4B (local GGUF model via ctransformers)
    - **AWS EKS Deployment**: Amazon Bedrock Nova Pro (`amazon.nova-pro-v1:0`) - **NEW (November 2025)**
- **Frontend Libraries**:
    - React
    - TypeScript
    - Vite
