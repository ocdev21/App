...
echo "  1. Health check: oc exec -it deployment/tslam-simple -n $PROJECT_NAME -- curl http://tslam-simple-service:8000/health"
echo "  2. Test L1 analysis: oc exec -it deployment/tslam-simple -n $PROJECT_NAME -- curl -X POST http://tslam-simple-service:8000/v1/chat/completions \\"
echo "                            -H 'Content-Type: application/json' \\"
echo "                            -d '{\"model\": \"tslam-4b\", \"messages\": [{\"role\": \"user\", \"content\": \"Analyze packet loss\"}]}'"
echo ""
echo "Or test from outside cluster:"
